...
function toggleEventAttendance($event_id)
{
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt->bind_param("ii", $event_id, $user_id);
        $stmt->execute();
        $response['message'] = "<span class='lang-en'>User removed from attendees list.</span><span class='lang-de'>Benutzer aus der Teilnehmerliste entfernt.</span>";
    } else {
        ...
        $stmt->bind_param("ii", $event_id, $user_id);
        $stmt->execute();
        $response['message'] = "<span class='lang-en'>User added to attendees list.</span><span class='lang-de'>Benutzer zur Teilnehmerliste hinzugefügt.</span>";
    }
...
function getEventAttendees($event_id)
{
    ...
    $query = "SELECT attendees.attendee_id, users.name, users.city, users.email 
              FROM attendees 
